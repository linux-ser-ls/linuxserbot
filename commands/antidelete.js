...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    if (!message.key.fromMe && !isOwner) {
        return sock.sendMessage(chatId, { text: '*ᴏɴʟʏ ᴛʜᴇ ʙᴏᴛ ᴏᴡɴᴇʀ ᴄᴀɴ ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ*' }, { quoted: message });
    }
...
📊 *Current status*: ${config.enabled ? '_✅ Enabled_' : '_❌ Disabled_'}

🛡️ *.antidelete on*
↳ _Enable antidelete protection_

🔓 *.antidelete off*
↳ _Disable antidelete protection_

🌷 _Keep deleted message visible_ ✨`
        }, {quoted: message});
    }
    ...
    if (match === 'on') {
        ...
        return sock.sendMessage(chatId, { text: '*ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ. ᴜꜱᴇ .ᴀɴᴛɪᴅᴇʟᴇᴛᴇ ᴛᴏ ꜱᴇᴇ ᴜꜱᴀɢᴇ*' }, {quoted:message});
    }

    saveAntideleteConfig(config);
    return sock.sendMessage(chatId, { text: `_Antidelete ${match === 'on' ? '✅ Enabled' : '❌ Disabled'}_` }, {quoted:message});
}
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        let text = `*🔰 ᴀɴᴛɪᴅᴇʟᴇᴛᴇ ʀᴇᴩᴏʀᴛ 🔰*\n\n` +
            `*🗑️ ᴅᴇʟᴇᴛᴇᴅ ʙʏ:* @${deletedBy.split('@')[0]}\n` +
            `*👤 ꜱᴇɴᴅᴇʀ:* @${senderName}\n` +
            `*📱 ɴᴜᴍʙᴇʀ:* ${sender}\n` +
            `*🕒 ᴛɪᴍᴇ:* ${time}\n\n` +
            `🌷 _ᴅᴇʟᴇᴛᴇᴅ ᴍᴇꜱꜱᴀɢᴇ ᴅᴇᴛᴇᴄᴛᴇᴅ_ ✨`;

        if (groupName) text += `*👥 ɢʀᴏᴜᴩ:* ${groupName}\n`;
